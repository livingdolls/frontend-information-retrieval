...
const Preprocessing = ({data}) => {
    ...
    const handleRefresh = async () => {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/_textPreprocessing.php`)
        .then(res => res.json())
    ...
    return(
        <Layout judulmenu = {"Text Preprocessing"}>
              {loading 
                ?
                <MainButton disabled endIcon={<Refresh />} sx={{fontSize:"14px", backgroundColor:"#eee", marginBottom:1}}>Refresh...</MainButton>
                :
                <MainButton onClick={handleRefresh} endIcon={<Refresh />} sx={{fontSize:"14px", marginBottom:1}}>Refresh</MainButton>
              }
